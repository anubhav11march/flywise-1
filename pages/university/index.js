...
function Universities() {
  ...
  const callUniversityData = async () => {
    try {
      const call1 = await axios.get("https://api.flywise.in/api/allcourses?activeStatus=true");
  ...
  const getAllfilterdata = async () => {
    try {
      const call2 = await axios.get("https://api.flywise.in/api/allcourses?activeStatus=true", { params: sidefilterData });
  ...
  useEffect(() => {
    if (sidefilterData?.englishExam != undefined) {
      setsidefilterData({
        ...sidefilterData, ieltsReading: "", ieltsListening: "", ieltsWriting: "",
